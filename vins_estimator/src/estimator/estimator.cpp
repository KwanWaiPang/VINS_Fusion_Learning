...
void Estimator::setParameter()
{
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        tic[i] = TIC[i];
        ric[i] = RIC[i];
        cout << " exitrinsic cam " << i << endl  << ric[i] << endl << tic[i].transpose() << endl;//外参
    }
    ...
    featureTracker.readIntrinsicParameter(CAM_NAMES);//读取camera的内参
...
// ###################################################gwphku
//输入event之后就进行特征的跟踪
void Estimator::inputEVENT(double t, const cv::Mat &_img, const dvs_msgs::EventArray::ConstPtr &event)
{
    inputImageCnt++;
...
void Estimator::processMeasurements()//主要的处理流程
{
    while (1)
    {
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header)
{
    ...
    if(ESTIMATE_EXTRINSIC == 2)//若设置为2的时候，对外参进行初始化矫正（camera与IMU的外参）
    {
        ...
        if (frame_count != 0)
        {
            ...
            Matrix3d calib_ric;
            if (initial_ex_rotation.CalibrationExRotation(corres, pre_integrations[frame_count]->delta_q, calib_ric))//进行矫正。具体过程进去看源码
            {
